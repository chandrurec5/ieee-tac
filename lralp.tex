...
When $s_t=0$, $s_{t+1} = s_t$ with probability $1-p$ and $s_{t+1}=s_t+1$ with probability $p$,
while when $s_t = S-1$, $s_{t+1} = s_t$ with probability $1-q(a_t)$ and $s_{t+1} = s_t-1$ with probability $q(a_t)$.
...
Hence, these features allow us to test the robustness of our theory. 
We chose $k=4$, a low number, to counteract  that the MDP is small scale. 
% (i.e., we used $1, s,s^2$ and $s^3$ as basis vectors).\\
At any given state $s$ we formulate LRALPs to compute the approximate values $\hat{J}(s')$ at the next states $s'$ (except for states $s=0$ and $s=999$ each state has two next states). While formulating the LRALPs, we adopt two different constraint selection strategies namely LRA and CS, described below.
...
$u_{\star}(s)=\argmin_{a\in A} g_a(s)+\sum_{s'\in S}p_a(s,s')\hat{J}_{\star}(s')$, where $\star$ is either LRA or CS.  
...
\textbf{LRA}:  Here we let $c=e_{s'}$ and choose the constraints corresponding to state $s'$ to ensure feasibility of LRALP. \todoc{How come this ensures feasibility?}
We choose $5$ more constraints corresponding to states $1,200,400,600,800,999$ (uniformly spaced across the state space) and compute $\hat{J}_{LRA}=\hat{J}_{e_{s'}}$. 
...
The idea is that $c_{s'}$ assigns higher weights to nearby states, while $c_{s'}$ provides a reasonable approximation
to the ideal, but intractable distribution described in \citet{CS}.
\begin{comment}
