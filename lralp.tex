...
In examples similar to this one, one possibility is to quantize the range of $\phi$: While this may worsen
the approximation quality a bit, it creates the opportunity 
...
\textbf{CS}: Here we let $c_{s'}(s)=(1-\alpha)(\alpha)^{|s'-s|}$ and normalize $c_{s'}$, sample $m=6$ constraints randomly using $c_{s'}$ and compute $\hat{J}_{CS}=\hat{J}_{c_{s'}}$. Since the optimal sampling distribution is unknown, we sample from $c_{s'}$ which assigns higher weights to nearby states.
\begin{comment}
