...
elements, this can be calculated in $O( p )$ time, making the total cost of obtaining the constraint matrix to be $O(mkp)$ regardless of the values of $S$ and $A$. \todoc{Somewhere discuss that the cost of obtaining a policy will depend on $A$.
...
\todoc{I think in the code you square the state-cost.}
We ran our experiments for $S=1000$, $A=4$ with $q(1)=0.2$, $q(2)=0.4$, $q(3)=0.6$, $q(4)=0.8$, $p=0.2$ and $\alpha=1-\frac{1}{S}$. 
%The moderate size of $S=1000$ enabled us to compute the exact value of $J^*$ (the most expensive part of the computation).
%Following previous work (e.g., \cite{ALP}),
...
%where  LRA is the one based on the \emph{conic} covering idea discussed in \Cref{mainresults} preceding \Cref{conetheorm}, and CS is based on constraint sampling \cite{CS}. 
...
%Except for the corner states i.e., $s=0$ and $s=999$, each state $0<s<S-1$ has two next states namely $s'=s-1$ and $s'=s+1$. We formulate two separate LRALPs (or just one LRALP for $s=0$ and $s=S-1$) for next states. 
%We evaluate LRA and CS by comparing the \emph{lookahead} policies they generate. The lookahead policy is local in nature, and the action at state $s$ is obtained by computing the approximate value functions of the next states and selecting the action that leads to the larger estimated value. 
...
In LRA, 
%We now describe the constraint selection strategies
 % LRA and CS.\\
%\textbf{LRA}:  Here 
we let $c=e_{s'}$ and set $\cS_0 = \{1,200,400,600,800,999,s'\}$. That $s'\in \cS_0$ ensures feasibility. \todoc{How come this ensures feasibility?}
...
%\textbf{CS}: Here we let $c_{s'}(s)=\zeta (1-\alpha)(\alpha)^{|s'-s|}$ and choose $\zeta>0$ so that $c_{s'}^\top \one =1$. We then sample $m=6$ constraints randomly from $c_{s'}$ and compute $\hat{J}_{CS}=\hat{J}_{c_{s'}}$. 
